...
void dwt_configure(dwt_config_t *config)
{
    ...
    regval =  (CHAN_CTRL_TX_CHAN_MASK & ((uint32)(chan) << CHAN_CTRL_TX_CHAN_SHIFT)) | // Transmit Channel
              (CHAN_CTRL_RX_CHAN_MASK & ((uint32)(chan) << CHAN_CTRL_RX_CHAN_SHIFT)) | // Receive Channel
...
void dwt_writetxfctrl(uint16 txFrameLength, uint16 txBufferOffset, int ranging)
{
    ...
    uint32 reg32 = (uint32_t)(pdw1000local->txFCTRL) | (uint32_t)(txFrameLength) | ((uint32_t)(txBufferOffset) << TX_FCTRL_TXBOFFS_SHFT) | ((uint32_t)(ranging) << TX_FCTRL_TR_SHFT);
...
uint16 dwt_read16bitoffsetreg(int regFileID,int regOffset)
{
    ...
    regval = ((uint32_t)(buffer[1]) << 8) + (uint32_t)(buffer[0]) ;
    return regval ;
...
{
    uint16 reg = (((uint16_t)(ops_sel) << OTP_SF_OPS_SEL_SHFT) & OTP_SF_OPS_SEL_MASK) | OTP_SF_OPS_KICK; // Select defined OPS table and trigger its loading
...
uint32 _dwt_computetxpowersetting(uint32 ref_powerreg, int32 power_adj)
{
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        new_regval |= ((uint32) (((uint32_t)(new_da_attn) << 5) | (uint32)(new_mixer_gain))) << (i * 8);
    }

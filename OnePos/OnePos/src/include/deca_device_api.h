...
typedef struct
{
    uint32 chan ;           //!< channel number {1, 2, 3, 4, 5, 7 }
    uint32 prf ;            //!< Pulse Repetition Frequency {DWT_PRF_16M or DWT_PRF_64M}
    uint32 txPreambLength ; //!< DWT_PLEN_64..DWT_PLEN_4096
    uint32 rxPAC ;          //!< Acquisition Chunk Size (Relates to RX preamble length)
    uint32 txCode ;         //!< TX preamble code
    uint32 rxCode ;         //!< RX preamble code
    uint32 nsSFD ;          //!< Boolean should we use non-standard SFD for better performance
    uint32 dataRate ;       //!< Data Rate {DWT_BR_110K, DWT_BR_850K or DWT_BR_6M8}
    uint32 phrMode ;        //!< PHR mode {0x0 - standard DWT_PHRMODE_STD, 0x3 - extended frames DWT_PHRMODE_EXT}
    uint32 sfdTO ;         //!< SFD timeout value (in symbols)
} dwt_config_t ;
